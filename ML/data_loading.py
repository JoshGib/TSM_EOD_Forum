...
#load data
Fine_tune_sample = pd.read_csv("fine_tune_sample.csv") #After taking sample went back and edited those that could be salvaged 42 were complete nonsense. used test 
original_summary_200 = pd.read_csv("train_daily_summaries.csv")

#keep whats needed only
original_summary_200 = original_summary_200[["trade_day","combined_article_summaries"]]

#merge so only fine tunee sample dates are selected
merged_df = pd.merge(original_summary_200, Fine_tune_sample, on='trade_day', how='inner')
