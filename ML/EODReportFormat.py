...
df['first_url'] = df['url_list'].str[0]

df = df.sort_values('trade_day').reset_index(drop=True)

print(df[['trade_day', 'generated_eod_summary', 'overall_article_signal','first_url']].to_string(index=False))

df.drop(columns=['daily_summary_sentiment','daily_summary_sentiment_score',
                 'combined_article_summaries','urls','url_list']).to_csv('test_OGTexts_cleaned.csv',
